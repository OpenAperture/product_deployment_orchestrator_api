...
defmodule OpenAperture.ProductDeploymentOrchestratorApi.Notifications.Publisher do
  use GenServer
  ...
  def handle_cast({:hipchat, exchange_id, broker_id, payload}, state) do
    ...
    case publish(options, hipchat_queue, payload) do
      :ok -> Logger.debug("[ProductDeploymentOrchestratorApi][NotificationsPublisher] Successfully published HipChat notification")
      {:error, reason} -> Logger.error("[ProductDeploymentOrchestratorApi][NotificationsPublisher] Failed to publish HipChat notification:  #{inspect reason}")
    end
    {:noreply, state}
  end
end